...
(function(Il2Cpp3) {
  ...
  function getExpectedModuleNames() {
    ...
    switch (Process.platform) {
      case "linux":
        return [Android.apiLevel ? "liblogic.so" : "GameAssembly.so"];
      case "windows":
...
function getClassLoader() {
  return {
    Gravity: frida_java_bridge_default.use("android.view.Gravity"),
    TextView: frida_java_bridge_default.use("android.widget.TextView"),
    LinearLayout: frida_java_bridge_default.use("android.widget.LinearLayout"),
    FrameLayout: frida_java_bridge_default.use("android.widget.FrameLayout"),
    FrameLayout_LayoutParams: frida_java_bridge_default.use("android.widget.FrameLayout$LayoutParams"),
    ViewGroup_LayoutParams: frida_java_bridge_default.use("android.view.ViewGroup$LayoutParams"),
    LinearLayout_LayoutParams: frida_java_bridge_default.use("android.widget.LinearLayout$LayoutParams"),
    Color: frida_java_bridge_default.use("android.graphics.Color"),
    Paint: frida_java_bridge_default.use("android.graphics.Paint"),
    RectF: frida_java_bridge_default.use("android.graphics.RectF"),
    GradientDrawable: frida_java_bridge_default.use("android.graphics.drawable.GradientDrawable"),
    ActivityThread: frida_java_bridge_default.use("android.app.ActivityThread"),
    ActivityThread_ActivityClientRecord: frida_java_bridge_default.use("android.app.ActivityThread$ActivityClientRecord"),
    View: frida_java_bridge_default.use("android.view.View"),
    View_OnTouchListener: frida_java_bridge_default.use("android.view.View$OnTouchListener"),
    View_OnClickListener: frida_java_bridge_default.use("android.view.View$OnClickListener"),
    MotionEvent: frida_java_bridge_default.use("android.view.MotionEvent"),
    String: frida_java_bridge_default.use("java.lang.String"),
    ScrollView: frida_java_bridge_default.use("android.widget.ScrollView"),
    SeekBar: frida_java_bridge_default.use("android.widget.SeekBar"),
    Runnable: frida_java_bridge_default.use("java.lang.Runnable"),
    Resources: frida_java_bridge_default.use("android.content.res.Resources"),
    DisplayMetrics: frida_java_bridge_default.use("android.util.DisplayMetrics"),
    ViewGroup: frida_java_bridge_default.use("android.view.ViewGroup"),
    LinearGradient: frida_java_bridge_default.use("android.graphics.LinearGradient"),
    Matrix: frida_java_bridge_default.use("android.graphics.Matrix"),
    Shader_TileMode: frida_java_bridge_default.use("android.graphics.Shader$TileMode"),
    Handler: frida_java_bridge_default.use("android.os.Handler"),
    Looper: frida_java_bridge_default.use("android.os.Looper")
  };
}
function uid(prefix) {
  return `com.example.${prefix}_${Date.now()}_${Math.floor(Math.random() * 1e6)}`;
}
function pixelDensityToPixels(context, dp) {
  const density = context.getResources().getDisplayMetrics().density.value;
  return parseInt(String(dp * density), 10);
}
function getMainActivity(classLoader) {
  ...
  const activityClientRecord = frida_java_bridge_default.cast(
    mActivities.valueAt(0),
    classLoader.ActivityThread_ActivityClientRecord
  );
  return activityClientRecord.activity.value;
}
function runOnUiThread(activity, fn) {
  ...
  activity.runOnUiThread(RunnableImpl.$new());
}
function createDrawCanvas(classLoader, activity) {
  const commands = [];
  const paintCache = {};
  let intervalHandler = null;
  let intervalRunnable = null;
  function getPaint(color) {
    if (!paintCache[color]) {
      const p = classLoader.Paint.$new(1);
      p.setColor(classLoader.Color.parseColor(color));
      paintCache[color] = p;
    }
    return paintCache[color];
  }
  const STYLE_FILL = frida_java_bridge_default.use("android.graphics.Paint$Style").FILL.value;
  const STYLE_STROKE = frida_java_bridge_default.use("android.graphics.Paint$Style").STROKE.value;
  const DrawCanvasView = frida_java_bridge_default.registerClass({
    name: uid("DrawCanvasView"),
    superClass: classLoader.View,
    methods: {
      $init: [{
        returnType: "void",
      ...
      onDraw: {
        ...
        implementation(canvas) {
          ...
          for (const cmd of commands) {
            const paint = getPaint(cmd.color);
            paint.setStyle(cmd.filled === false ? STYLE_STROKE : STYLE_FILL);
            if (cmd.strokeWidth) paint.setStrokeWidth(cmd.strokeWidth);
            switch (cmd.type) {
              case "text":
                paint.setTextSize(cmd.size || 24);
                canvas.drawText(classLoader.String.$new(cmd.text), cmd.x, cmd.y, paint);
                break;
              case "line":
              ...
              case "rect":
                ...
                break;
              case "roundrect": {
                const rf = classLoader.RectF.$new(cmd.x, cmd.y, cmd.x + cmd.w, cmd.y + cmd.h);
                canvas.drawRoundRect(rf, cmd.rx, cmd.ry, paint);
                break;
              }
              case "circle":
  ...
  const view = DrawCanvasView.$new(activity);
  const canvasApi = {
    view,
    drawText(x, y, text, color = "#FFFFFF", size = 24) {
      commands.push({
        type: "text",
        x,
        y,
    ...
    drawLine(x1, y1, x2, y2, color = "#FFFFFF", strokeWidth = 2) {
      commands.push({
        type: "line",
        x1,
        y1,
    ...
    drawRoundRect(x, y, w, h, rx, ry, color = "#FFFFFF", filled = true) {
      commands.push({
        type: "roundrect",
        x,
        y,
    ...
    drawCircle(cx, cy, r, color = "#FFFFFF", filled = true) {
      commands.push({
        type: "circle",
        cx,
        cy,
    ...
    invalidate() {
      runOnUiThread(activity, () => view.invalidate());
    },
    // Starts the 500ms execution loop safely on the main thread
    startPeriodicUpdate(updateCallback) {
      this.stopPeriodicUpdate();
      const Handler = frida_java_bridge_default.use("android.os.Handler");
      const Looper = frida_java_bridge_default.use("android.os.Looper");
      intervalHandler = Handler.$new(Looper.getMainLooper());
      const TickerRunnable = frida_java_bridge_default.registerClass({
        name: uid("DrawCanvasTicker"),
        implements: [frida_java_bridge_default.use("java.lang.Runnable")],
        methods: {
          run: () => {
            canvasApi.clear();
            updateCallback(canvasApi);
            view.invalidate();
            intervalHandler.postDelayed(intervalRunnable, 500);
          }
        }
      });
      intervalRunnable = TickerRunnable.$new();
      intervalHandler.post(intervalRunnable);
    },
    // Stops the loop to prevent memory leaks when menu closes
    stopPeriodicUpdate() {
      if (intervalHandler && intervalRunnable) {
...
var Menu = class {
  ...
  #menuStart;
  #menuLayout;
  // FrameLayout, holds chrome + resize handle
  #menuChrome;
  // LinearLayout inside menuLayout: bar + scroll
  #menuBarLayout;
  #menuBarTitle;
  #menuScrollLayout;
  #menuScrollView;
  ...
  #createContentView() {
    const c = this.#classLoader;
    this.#contentView = c.FrameLayout.$new(this.#activity);
    const lp = c.FrameLayout_LayoutParams.$new(this.#MATCH_PARENT, this.#MATCH_PARENT);
    this.#contentView.setLayoutParams(lp);
    this.#contentView.setBackgroundColor(c.Color.TRANSPARENT.value);
  }
  #createMainLayout() {
    const c = this.#classLoader;
    const lp = c.LinearLayout_LayoutParams.$new(this.#WRAP_CONTENT, this.#WRAP_CONTENT);
    this.#mainLayout = c.LinearLayout.$new(this.#activity);
    this.#mainLayout.setLayoutParams(lp);
  }
  #createMenuScroll() {
    const c = this.#classLoader;
    const lp = c.LinearLayout_LayoutParams.$new(this.#MATCH_PARENT, this.#WRAP_CONTENT);
    this.#menuScrollView = c.ScrollView.$new(this.#activity);
    const padding = pixelDensityToPixels(this.#activity, 8);
    this.#menuScrollView.setLayoutParams(lp);
  ...
  #createMenuScrollLayout() {
    const c = this.#classLoader;
    const lp = c.LinearLayout_LayoutParams.$new(this.#MATCH_PARENT, this.#WRAP_CONTENT);
    this.#menuScrollLayout = c.LinearLayout.$new(this.#activity);
    this.#menuScrollLayout.setLayoutParams(lp);
  ...
  createMenuStart(title, size, color) {
    ...
    const sizeDp = pixelDensityToPixels(activity, size);
    const lp = c.LinearLayout_LayoutParams.$new(
      this.#WRAP_CONTENT,
      this.#WRAP_CONTENT
    );
    this.#menuStart = c.TextView.$new(activity);
    this.#menuStart.setLayoutParams(lp);
    this.#menuStart.setText(c.String.$new(title));
    this.#menuStart.setTextSize(sizeDp);
    this.#menuStart.setTextColor(c.Color.WHITE.value);
    const GRADIENT_WIDTH = 700;
    const colors = frida_java_bridge_default.array("int", [
      c.Color.parseColor("#00F5FF"),
      c.Color.parseColor("#0099FF"),
      c.Color.parseColor("#004CFF"),
      c.Color.parseColor("#6A00FF"),
      c.Color.parseColor("#AA00FF"),
      c.Color.parseColor("#FF00FF"),
      c.Color.parseColor("#FF0088"),
      c.Color.parseColor("#FF0033"),
      c.Color.parseColor("#FF6600"),
      c.Color.parseColor("#FFD500"),
      c.Color.parseColor("#7CFF00"),
      c.Color.parseColor("#00FF88"),
      c.Color.parseColor("#00F5FF")
    ]);
    const shader = c.LinearGradient.$new(
      0,
      0,
      GRADIENT_WIDTH,
      0,
      colors,
      null,
      c.Shader_TileMode.REPEAT.value
    );
    const matrix = c.Matrix.$new();
    const handler = c.Handler.$new(c.Looper.getMainLooper());
    let offset = 0;
    const self = this.#menuStart;
    self.setLayerType(1, null);
    const Animator = frida_java_bridge_default.registerClass({
      name: uid("NeonWaverAnimator"),
      implements: [c.Runnable],
      methods: {
        run() {
          offset = (offset + 3) % GRADIENT_WIDTH;
          const angleRad = offset / GRADIENT_WIDTH * 2 * Math.PI;
          matrix.reset();
          matrix.postRotate(Math.sin(angleRad) * 4, GRADIENT_WIDTH / 2, 0);
          matrix.postTranslate(offset, Math.sin(angleRad * 2) * 12);
          shader.setLocalMatrix(matrix);
          const paint = self.getPaint();
          paint.setShader(shader);
          const waveY = Math.sin(angleRad) * 16;
          self.setTranslationY(waveY);
          const pulseScale = 1 + Math.sin(angleRad * 2) * 0.04;
          self.setScaleX(pulseScale);
          self.setScaleY(pulseScale);
          const horizontalSkew = Math.cos(angleRad) * 0.06;
          paint.setTextSkewX(horizontalSkew);
          const brightness = 0.85 + 0.15 * Math.sin(angleRad);
          const hsv = frida_java_bridge_default.array("float", [
            offset / GRADIENT_WIDTH * 360,
            0.9,
            brightness
          ]);
          const glow = c.Color.HSVToColor(hsv);
          const glowSize = 14 + Math.sin(angleRad * 2) * 6;
          paint.setShadowLayer(glowSize, 0, 0, glow);
          self.invalidate();
          handler.postDelayed(this, 16);
        }
      }
    });
    handler.post(Animator.$new());
  }
  // width/height in dp. Menu is now resizable, so it's no longer forced square.
  createMenuLayout(color, width, height) {
    const c = this.#classLoader;
    const activity = this.#activity;
    this.#menuWidth = pixelDensityToPixels(activity, width);
    this.#menuHeight = pixelDensityToPixels(activity, height);
    this.#menuLayout = c.FrameLayout.$new(activity);
    const lp = c.LinearLayout_LayoutParams.$new(this.#menuWidth, this.#menuHeight);
    this.#menuLayout.setLayoutParams(lp);
    const bg = c.GradientDrawable.$new();
    bg.setColor(c.Color.parseColor(color));
    bg.setCornerRadius(pixelDensityToPixels(activity, 14));
    this.#menuLayout.setBackground(bg);
    this.#menuLayout.setElevation(pixelDensityToPixels(activity, 6));
    this.#menuChrome = c.LinearLayout.$new(activity);
    this.#menuChrome.setOrientation(this.#menuChrome.VERTICAL.value);
    const chromeParams = c.FrameLayout_LayoutParams.$new(this.#MATCH_PARENT, this.#MATCH_PARENT);
    this.#menuChrome.setLayoutParams(chromeParams);
    this.#menuLayout.addView(this.#menuChrome);
  ...
  createMenuBarLayout(color) {
    ...
    const padding = pixelDensityToPixels(this.#activity, 8);
    const lp = c.LinearLayout_LayoutParams.$new(this.#MATCH_PARENT, this.#WRAP_CONTENT);
    this.#menuBarLayout = c.LinearLayout.$new(this.#activity);
    this.#menuBarLayout.setLayoutParams(lp);
    this.#menuBarLayout.setBackgroundColor(c.Color.parseColor(color));
    this.#menuBarLayout.setPadding(padding, padding, 0, padding);
  }
  createMenuBarTitle(title, size, color) {
    const c = this.#classLoader;
    const activity = this.#activity;
    const sizeDp = pixelDensityToPixels(activity, size);
    const lp = c.LinearLayout_LayoutParams.$new(
    ...
    this.#menuBarTitle = c.TextView.$new(activity);
    this.#menuBarTitle.setLayoutParams(lp);
    this.#menuBarTitle.setText(c.String.$new(title));
    this.#menuBarTitle.setTextSize(sizeDp);
    this.#menuBarTitle.setTextColor(c.Color.WHITE.value);
    const GRADIENT_WIDTH = 700;
    const colors = frida_java_bridge_default.array("int", [
      c.Color.parseColor("#00F5FF"),
      c.Color.parseColor("#0099FF"),
    ...
    const handler = c.Handler.$new(c.Looper.getMainLooper());
    let offset = 0;
    const self = this.#menuBarTitle;
    self.setLayerType(1, null);
    const Animator = frida_java_bridge_default.registerClass({
      name: uid("NeonMenuBarAnimator"),
      implements: [c.Runnable],
      methods: {
        run() {
          offset = (offset + 3) % GRADIENT_WIDTH;
  ...
  // Bottom-left resize handle, ImGui style. Drag left = wider, drag down = taller,
  // top-right corner of the menu stays anchored (mainLayout.X compensates for width delta).
  #addResizeHandle() {
    const c = this.#classLoader;
    const activity = this.#activity;
    const handleSizePx = pixelDensityToPixels(activity, 16);
    const minWidth = pixelDensityToPixels(activity, 160);
    const minHeight = pixelDensityToPixels(activity, 120);
    const handle = c.View.$new(activity);
    const hp = c.FrameLayout_LayoutParams.$new(handleSizePx, handleSizePx);
    hp.gravity.value = c.Gravity.BOTTOM.value | c.Gravity.END.value;
    handle.setLayoutParams(hp);
    const Path = frida_java_bridge_default.use("android.graphics.Path");
    const PathShape = frida_java_bridge_default.use("android.graphics.drawable.shapes.PathShape");
    const ShapeDrawable = frida_java_bridge_default.use("android.graphics.drawable.ShapeDrawable");
    const trianglePath = Path.$new();
    trianglePath.moveTo(handleSizePx, 0);
    trianglePath.lineTo(handleSizePx, handleSizePx);
    trianglePath.lineTo(0, handleSizePx);
    trianglePath.close();
    const triangleShape = PathShape.$new(trianglePath, handleSizePx, handleSizePx);
    const imguiGripDrawable = ShapeDrawable.$new(triangleShape);
    const paint = imguiGripDrawable.getPaint();
    paint.setColor(c.Color.parseColor("#9946505A"));
    handle.setBackground(frida_java_bridge_default.cast(imguiGripDrawable, frida_java_bridge_default.use("android.graphics.drawable.Drawable")));
    let initialWidth = 0;
    let initialHeight = 0;
    let initialTouchX = 0;
    let initialTouchY = 0;
    let fixedMenuX = 0;
    let fixedMenuY = 0;
    let maxWidthAllowed = minWidth;
    let maxHeightAllowed = minHeight;
    const menuLayout = this.#menuLayout;
    const mainLayout = this.#mainLayout;
    const ResizeTouchListener = frida_java_bridge_default.registerClass({
      name: uid("ResizeHandle"),
      implements: [c.View_OnTouchListener],
      methods: {
        onTouch(_view, event) {
          switch (event.getAction()) {
            case c.MotionEvent.ACTION_DOWN.value: {
              const lp = menuLayout.getLayoutParams();
              if (lp.gravity) {
                lp.gravity.value = c.Gravity.TOP.value | c.Gravity.START.value;
              }
              initialWidth = lp.width.value;
              initialHeight = lp.height.value;
              initialTouchX = event.getRawX();
              initialTouchY = event.getRawY();
              fixedMenuX = mainLayout.getX();
              fixedMenuY = mainLayout.getY();
              const parent = frida_java_bridge_default.cast(mainLayout.getParent(), c.ViewGroup);
              const parentWidth = parent.getWidth();
              const parentHeight = parent.getHeight();
              maxWidthAllowed = Math.max(minWidth, parentWidth - fixedMenuX);
              maxHeightAllowed = Math.max(minHeight, parentHeight - fixedMenuY);
              menuLayout.setLayoutParams(lp);
              break;
            }
            case c.MotionEvent.ACTION_MOVE.value: {
              const deltaX = event.getRawX() - initialTouchX;
              const deltaY = event.getRawY() - initialTouchY;
              let newWidth = Math.max(initialWidth + deltaX, minWidth);
              newWidth = Math.min(newWidth, maxWidthAllowed);
              let newHeight = Math.max(initialHeight + deltaY, minHeight);
              newHeight = Math.min(newHeight, maxHeightAllowed);
              const lp = menuLayout.getLayoutParams();
              lp.width.value = Math.floor(newWidth);
              lp.height.value = Math.floor(newHeight);
              menuLayout.setLayoutParams(lp);
              mainLayout.setX(fixedMenuX);
              mainLayout.setY(fixedMenuY);
              break;
            }
            default:
              return false;
    ...
    handle.setOnTouchListener(ResizeTouchListener.$new());
    this.#menuLayout.addView(handle);
  }
  #drawContentView() {
  ...
  #drawMenuBarLayout() {
    this.#menuChrome.addView(this.#menuBarLayout);
  }
  #drawMenuBarTitle() {
    this.#menuBarLayout.addView(this.#menuBarTitle);
  }
  #drawMenuOptions() {
    this.#menuChrome.addView(this.#menuScrollView);
  ...
  #createOptionClickEvent(id, optionView, callbacks) {
    ...
    const OptionOnClickListener = frida_java_bridge_default.registerClass({
      name: uid(id),
      implements: [c.View_OnClickListener],
      methods: {
        onClick(p1) {
          if (!optionState) {
            p1.setBackgroundColor(c.Color.parseColor(colorOn));
            optionState = true;
            callbacks.on();
          } else {
            p1.setBackgroundColor(c.Color.parseColor(colorOff));
            optionState = false;
            callbacks.off();
          }
        }
  ...
  addOption(id, name, callbacks, targetContainer = this.#menuScrollLayout) {
    const c = this.#classLoader;
    const lp = c.LinearLayout_LayoutParams.$new(this.#MATCH_PARENT, this.#WRAP_CONTENT);
    const padding = pixelDensityToPixels(this.#activity, 5);
    const option = c.TextView.$new(this.#activity);
    const margin = pixelDensityToPixels(this.#activity, 10);
    option.setText(c.String.$new(name));
    option.setBackgroundColor(c.Color.parseColor(this.#colorOff));
    option.setTextColor(c.Color.parseColor("#75757B"));
    lp.setMargins(0, 0, 0, margin);
    option.setLayoutParams(lp);
    option.setPadding(padding, padding, 0, padding);
    targetContainer.addView(option);
    this.#createOptionClickEvent(id, option, callbacks);
    return option;
  }
  // `targetContainer` defaults to the main options list; createDynamicSection()
  // passes its own sub-container so a subset of widgets can be cleared/rebuilt
  // independently of the rest of the menu.
  addText(text, textSize, textColor, targetContainer = this.#menuScrollLayout) {
    const c = this.#classLoader;
    const lp = c.LinearLayout_LayoutParams.$new(this.#WRAP_CONTENT, this.#WRAP_CONTENT);
    const margin = pixelDensityToPixels(this.#activity, 5);
    const textView = c.TextView.$new(this.#activity);
    textView.setText(c.String.$new(text));
    textView.setTextSize(textSize);
    textView.setTextColor(c.Color.parseColor(textColor));
    lp.setMargins(0, 0, 0, margin);
  ...
  addSeekBar(textValue, initialValue, minValue, maxValue, callback) {
    ...
    const seekBar = c.SeekBar.$new(this.#activity);
    const textView = c.TextView.$new(this.#activity);
    seekBar.setMax(maxValue - minValue);
    seekBar.setProgress(0);
    lp.setMargins(0, 0, 0, margin);
    seekBar.setLayoutParams(lp);
    textView.setText(c.String.$new(`${textValue} ${initialValue}`));
    textView.setTextColor(c.Color.parseColor("#75757B"));
    seekBar.setProgress(initialValue);
    const SeekBarChangeListener = frida_java_bridge_default.use("android.widget.SeekBar$OnSeekBarChangeListener");
    const SeekBarChangeListenerImplementation = frida_java_bridge_default.registerClass({
      name: uid("SeekBarChangeListener"),
      implements: [SeekBarChangeListener],
      methods: {
        onProgressChanged(_seekBar, progress, _fromUser) {
          const value = progress + minValue;
          textView.setText(c.String.$new(`${textValue} ${value}`));
          callback(value, "move");
        },
        onStartTrackingTouch(seekBarInstance) {
          const value = seekBarInstance.getProgress() + minValue;
          textView.setText(c.String.$new(`${textValue} ${value}`));
          callback(value, "start");
        },
        onStopTrackingTouch(seekBarInstance) {
          const value = seekBarInstance.getProgress() + minValue;
          textView.setText(c.String.$new(`${textValue} ${value}`));
          callback(value, "end");
        }
    ...
    textView.setLayoutParams(lp);
    textView.setGravity(c.Gravity.CENTER.value);
  }
  // ImGui-style toggle switch, drawn with Canvas (not the stock android.widget.Switch).
  // Returns { get, set } so you can read/force state from hook callbacks.
  addSwitch(label, initialState, onChange, targetContainer = this.#menuScrollLayout) {
    const c = this.#classLoader;
    const activity = this.#activity;
    const container = c.LinearLayout.$new(activity);
    const containerParams = c.LinearLayout_LayoutParams.$new(this.#MATCH_PARENT, this.#WRAP_CONTENT);
    containerParams.setMargins(0, 0, 0, pixelDensityToPixels(activity, 8));
    container.setLayoutParams(containerParams);
    container.setOrientation(container.HORIZONTAL.value);
    container.setGravity(c.Gravity.CENTER_VERTICAL.value);
    const labelView = c.TextView.$new(activity);
    const labelParams = c.LinearLayout_LayoutParams.$new(0, this.#WRAP_CONTENT, 1);
    labelView.setLayoutParams(labelParams);
    labelView.setText(c.String.$new(label));
    labelView.setTextColor(c.Color.parseColor("#E4E4E7"));
    let state = !!initialState;
    const trackW = pixelDensityToPixels(activity, 46);
    const trackH = pixelDensityToPixels(activity, 26);
    const thumbR = pixelDensityToPixels(activity, 11);
    const SwitchView = frida_java_bridge_default.registerClass({
      name: uid("SwitchView"),
      superClass: c.View,
      methods: {
        $init: [{
        ...
        onDraw: {
          ...
          implementation(canvas) {
            this.$super.onDraw(canvas);
            const trackPaint = c.Paint.$new(1);
            trackPaint.setColor(c.Color.parseColor(state ? "#34C759" : "#3A3A3F"));
            const rf = c.RectF.$new(0, 0, trackW, trackH);
            canvas.drawRoundRect(rf, trackH / 2, trackH / 2, trackPaint);
            const thumbPaint = c.Paint.$new(1);
            thumbPaint.setColor(c.Color.parseColor("#FFFFFF"));
            const cx = state ? trackW - trackH / 2 : trackH / 2;
            canvas.drawCircle(cx, trackH / 2, thumbR, thumbPaint);
          }
        }
      }
    });
    const switchView = SwitchView.$new(activity);
    switchView.setLayoutParams(c.LinearLayout_LayoutParams.$new(trackW, trackH));
    const ClickListener = frida_java_bridge_default.registerClass({
      name: uid("SwitchClick"),
      implements: [c.View_OnClickListener],
      methods: {
        onClick(_v) {
          state = !state;
    ...
    switchView.setOnClickListener(ClickListener.$new());
    ...
    return {
      ...
      set: (value) => {
        state = !!value;
        runOnUiThread(activity, () => switchView.invalidate());
      }
    };
  }
  // Switch + numeric stepper (-/+). Stepper is dimmed/disabled while switch is off.
  // Returns { getEnabled, getCount, setCount }.
  // `targetContainer` lets createDynamicSection() route these into a rebuildable
  // sub-list (e.g. one row per recommended hero) instead of the main options list.
  addSwitchWithCounter(label, initialState, initialCount, minCount, maxCount, onChange, targetContainer = this.#menuScrollLayout) {
    const c = this.#classLoader;
    const activity = this.#activity;
    const container = c.LinearLayout.$new(activity);
    const containerParams = c.LinearLayout_LayoutParams.$new(this.#MATCH_PARENT, this.#WRAP_CONTENT);
    ...
    container.setGravity(c.Gravity.CENTER_VERTICAL.value);
    const labelView = c.TextView.$new(activity);
    labelView.setLayoutParams(c.LinearLayout_LayoutParams.$new(0, this.#WRAP_CONTENT, 1));
    labelView.setText(c.String.$new(label));
    labelView.setTextColor(c.Color.parseColor("#FFFFFF"));
    let enabled = !!initialState;
    let count = initialCount;
    const minusBtn = c.TextView.$new(activity);
    minusBtn.setText(c.String.$new("-"));
    minusBtn.setTextColor(c.Color.parseColor("#FFFFFF"));
    minusBtn.setPadding(
    ...
    const countView = c.TextView.$new(activity);
    countView.setText(c.String.$new(String(count)));
    countView.setTextColor(c.Color.parseColor("#FFFFFF"));
    countView.setGravity(c.Gravity.CENTER.value);
    countView.setLayoutParams(c.LinearLayout_LayoutParams.$new(pixelDensityToPixels(activity, 30), this.#WRAP_CONTENT));
    const plusBtn = c.TextView.$new(activity);
    plusBtn.setText(c.String.$new("+"));
    plusBtn.setTextColor(c.Color.parseColor("#FFFFFF"));
    plusBtn.setPadding(
      pixelDensityToPixels(activity, 10),
      0,
      pixelDensityToPixels(activity, 10),
    ...
    function refreshStepperVisual() {
      const alpha = enabled ? 1 : 0.35;
      minusBtn.setAlpha(alpha);
      plusBtn.setAlpha(alpha);
      countView.setAlpha(alpha);
    }
    function setCount(value) {
      count = Math.min(maxCount, Math.max(minCount, value));
      countView.setText(c.String.$new(String(count)));
      onChange(enabled, count);
    }
    const MinusClick = frida_java_bridge_default.registerClass({
      name: uid("CounterMinus"),
      implements: [c.View_OnClickListener],
      methods: {
        onClick: () => {
          if (enabled) setCount(count - 1);
        }
      }
    });
    const PlusClick = frida_java_bridge_default.registerClass({
      name: uid("CounterPlus"),
      implements: [c.View_OnClickListener],
      methods: {
        onClick: () => {
          if (enabled) setCount(count + 1);
    ...
    minusBtn.setOnClickListener(MinusClick.$new());
    plusBtn.setOnClickListener(PlusClick.$new());
    const trackW = pixelDensityToPixels(activity, 46);
    const trackH = pixelDensityToPixels(activity, 26);
    const thumbR = pixelDensityToPixels(activity, 11);
    const SwitchView = frida_java_bridge_default.registerClass({
      name: uid("SwitchWithCounterView"),
      superClass: c.View,
      methods: {
        $init: [{
          returnType: "void",
        ...
        onDraw: {
          ...
          implementation(canvas) {
            this.$super.onDraw(canvas);
            const trackPaint = c.Paint.$new(1);
            trackPaint.setColor(c.Color.parseColor(enabled ? "#34C759" : "#3A3A3F"));
            const rf = c.RectF.$new(0, 0, trackW, trackH);
            canvas.drawRoundRect(rf, trackH / 2, trackH / 2, trackPaint);
            const thumbPaint = c.Paint.$new(1);
            thumbPaint.setColor(c.Color.parseColor("#FFFFFF"));
            const cx = enabled ? trackW - trackH / 2 : trackH / 2;
            canvas.drawCircle(cx, trackH / 2, thumbR, thumbPaint);
          }
        }
      }
    ...
    const switchView = SwitchView.$new(activity);
    switchView.setLayoutParams(c.LinearLayout_LayoutParams.$new(trackW, trackH));
    const ToggleClick = frida_java_bridge_default.registerClass({
      name: uid("SwitchWithCounterClick"),
      implements: [c.View_OnClickListener],
      methods: {
        onClick: () => {
          enabled = !enabled;
          switchView.invalidate();
          refreshStepperVisual();
          onChange(enabled, count);
        }
      }
    });
    switchView.setOnClickListener(ToggleClick.$new());
    refreshStepperVisual();
    container.addView(labelView);
  ...
  // Add an ImGui-style immediate-mode draw surface inside the menu, e.g. for a
  // custom graph/HUD. widthDp/heightDp size the canvas; returns the draw API
  // from createDrawCanvas() so hooks can push shapes into it.
  addDrawCanvas(widthDp, heightDp) {
    const activity = this.#activity;
    const canvasApi = createDrawCanvas(this.#classLoader, activity);
    const lp = this.#classLoader.LinearLayout_LayoutParams.$new(
      pixelDensityToPixels(activity, widthDp),
      pixelDensityToPixels(activity, heightDp)
    );
    canvasApi.view.setLayoutParams(lp);
    this.#menuScrollLayout.addView(canvasApi.view);
    return canvasApi;
  }
  // Creates an empty sub-container inside the options list that can be cleared
  // and repopulated independently of the rest of the menu — e.g. a hero list
  // that changes size every refresh. Returns a scoped API mirroring the widget
  // methods above (addText, addOption, addSwitch, addSwitchWithCounter), plus
  // clear(). All calls must happen on the UI thread, same as the rest of Menu.
  createDynamicSection() {
    ...
    container.setOrientation(container.VERTICAL.value);
    this.#menuScrollLayout.addView(container);
    const self = this;
    return {
      clear() {
      ...
      addText(text, textSize, textColor) {
        return self.addText(text, textSize, textColor, container);
      },
      addOption(id, name, callbacks) {
        return self.addOption(id, name, callbacks, container);
      },
      addSwitch(label, initialState, onChange) {
        return self.addSwitch(label, initialState, onChange, container);
      },
      addSwitchWithCounter(label, initialState, initialCount, minCount, maxCount, onChange) {
        return self.addSwitchWithCounter(label, initialState, initialCount, minCount, maxCount, onChange, container);
      }
    };
  }
  #createMainLayoutEvent() {
    ...
    const MainLayoutOnTouchListener = frida_java_bridge_default.registerClass({
      name: uid("MainLayoutEvent"),
      implements: [c.View_OnTouchListener],
      methods: {
        onTouch(view, event) {
          switch (event.getAction()) {
            case c.MotionEvent.ACTION_DOWN.value:
              ...
              break;
            case c.MotionEvent.ACTION_UP.value:
              if (!isMove) {
                if (!isMenuLayout) {
                  self.#iconX = mainLayout.getX();
                  self.#iconY = mainLayout.getY();
                  mainLayout.removeView(menuStart);
                  mainLayout.addView(menuLayout);
                  mainLayout.post(frida_java_bridge_default.registerClass({
                    name: uid("PostOpen"),
                    implements: [c.Runnable],
                    methods: {
                      run() {
                        mainLayout.setX(self.#menuX);
                        mainLayout.setY(self.#menuY);
                      }
                    }
                  }).$new());
                  isMenuLayout = true;
                } else {
                  self.#menuX = mainLayout.getX();
                  self.#menuY = mainLayout.getY();
                  ...
                  mainLayout.post(frida_java_bridge_default.registerClass({
                    ...
                    methods: {
                      run() {
                        mainLayout.setX(self.#iconX);
                        mainLayout.setY(self.#iconY);
                      }
                    }
                  }).$new());
                  isMenuLayout = false;
              ...
              break;
            case c.MotionEvent.ACTION_MOVE.value: {
              let targetX = event.getRawX() + initialX;
              let targetY = event.getRawY() + initialY;
              const parent = frida_java_bridge_default.cast(view.getParent(), c.ViewGroup);
              const parentWidth = parent.getWidth();
              const parentHeight = parent.getHeight();
              const viewWidth = view.getWidth();
              const viewHeight = view.getHeight();
              if (targetX < 0) targetX = 0;
              if (targetX + viewWidth > parentWidth) targetX = parentWidth - viewWidth;
              if (targetY < 0) targetY = 0;
              if (targetY + viewHeight > parentHeight) targetY = parentHeight - viewHeight;
              view.setX(targetX);
              view.setY(targetY);
              if (isMenuLayout) {
                self.#menuX = view.getX();
                self.#menuY = view.getY();
              } else {
                self.#iconX = view.getX();
                self.#iconY = view.getY();
              }
  ...
  // Accepts explicit widthPx/heightPx (e.g. screen dimensions from DisplayMetrics).
  // Falls back to MATCH_PARENT if omitted.
  addOverlayCanvas(widthPx, heightPx) {
    const activity = this.#activity;
    const canvasApi = createDrawCanvas(this.#classLoader, activity);
    const w = typeof widthPx === "number" ? widthPx : this.#MATCH_PARENT;
    const h = typeof heightPx === "number" ? heightPx : this.#MATCH_PARENT;
    const lp = this.#classLoader.FrameLayout_LayoutParams.$new(w, h);
    canvasApi.view.setLayoutParams(lp);
    canvasApi.view.setBackgroundColor(
      this.#classLoader.Color.TRANSPARENT.value
    );
    activity.addContentView(canvasApi.view, lp);
    this.#overlayViews.push(canvasApi);
    return canvasApi;
  }
  start() {
    ...
    runOnUiThread(this.#activity, () => {
      this.#mainLayout.setX(this.#iconX);
      this.#mainLayout.setY(this.#iconY);
    });
  }
  destroy() {
    runOnUiThread(this.#activity, () => {
      try {
        for (const overlay of this.#overlayViews) {
          try {
            overlay.stopPeriodicUpdate?.();
          } catch (e) {
          }
          try {
            const parent = overlay.view.getParent();
            if (parent)
              frida_java_bridge_default.cast(
                parent,
                this.#classLoader.ViewGroup
              ).removeView(overlay.view);
          } catch (e) {
          }
        }
        this.#overlayViews.length = 0;
        if (this.#contentView) {
          const parent = this.#contentView.getParent();
          if (parent) {
            frida_java_bridge_default.cast(
              parent,
              this.#classLoader.ViewGroup
        ...
        console.log("Menu destroy failed: " + e);
      }
    });
  }
};
...
var KEEP_COIN = 20;
var COPIES_2_STAR = 3;
var PriorityHeroTargets = /* @__PURE__ */ new Map();
var heroMap = /* @__PURE__ */ new Map();
var isAuthorized = true;
var menu = null;
var hud = null;
var heroSection = null;
var defaultCopiesTarget = COPIES_2_STAR;
var isRefreshing = false;
var nextEnemyName = "Waiting ...";
var hooksReady = false;
var _cl = null;
function getHeroElements(lib2) {
  if (heroMap.size > 0)
    return heroMap;
  const cls = lib2.class("CData_MCHero_Element");
  const instances = Il2Cpp.gc.choose(cls);
  ...
  for (const instance of instances) {
    try {
      if (instance.isNull())
        continue;
      const m_ID = instance.field("m_ID").value;
      if (!m_ID || m_ID <= 0)
        continue;
      const nameField = instance.field("m_mName").value;
      if (nameField.isNull())
        continue;
      const rawName = nameField.content ?? "unknown";
      if (!rawName || rawName === "unknown")
        continue;
      heroMap.set(m_ID, {
        m_ID,
        m_mName: rawName.trim().toLowerCase(),
        m_Quality: instance.field("m_Quality").value,
        m_IsTank: instance.field("m_IsTank").value,
        m_occupation: instance.field("m_occupation").value,
        m_AttackType: instance.field("m_AttackType").value,
        m_HeroType: instance.field("m_HeroType").value,
        m_HeroGroup: instance.field("m_HeroGroup").value
      });
    } catch (e) {
    }
  }
...
var BattleDataClass;
var HeroShopClass;
var BattleBridgeClass;
var battleBridge = null;
var battleData = null;
function getOpponent() {
  if (!hooksReady || !battleData)
    return "Waiting ...";
  try {
    const accountId = BattleDataClass.field("m_SelfAccID").value;
    if (!accountId)
      return "Waiting ...";
    const opponentId = battleData.method("ILOGIC_GetCurrentOpponentAccountID").invoke(accountId);
    const opponentName = battleData.method("ILOGIC_GetSelfChessPlayerName").invoke(opponentId);
    return opponentName ?? "Unknown";
  } catch (e) {
    return "Waiting ...";
  }
}
function refreshHeroList() {
  if (!heroSection || !hooksReady)
    return;
  if (isRefreshing)
    return;
  isRefreshing = true;
  frida_java_bridge_default.scheduleOnMainThread(() => {
    heroSection.clear();
    heroSection.addText("Loading...", 16, "#FFFF00");
  });
  setTimeout(() => {
    try {
      if (heroMap.size === 0)
      ...
      battleBridge = null;
      try {
        const [foundBridge] = Il2Cpp.gc.choose(BattleBridgeClass);
        battleBridge = foundBridge ?? null;
      } catch (e) {
        battleBridge = null;
      }
      const rowsToBuild = [];
      if (heroMap.size > 0 && battleBridge) {
        const addedNames = /* @__PURE__ */ new Set();
        const sortedHeroes = Array.from(heroMap.values()).sort((a, b) => a.m_mName.localeCompare(b.m_mName));
        for (const hero of sortedHeroes) {
          if (addedNames.has(hero.m_mName))
            continue;
          let isRecommendedLineUp = false;
          try {
            isRecommendedLineUp = battleBridge.method("IsHeroInRecommendLineup").invoke(hero.m_ID);
          } catch (e) {
            continue;
          }
          if (!isRecommendedLineUp)
            continue;
          addedNames.add(hero.m_mName);
          const isTracked = PriorityHeroTargets.has(hero.m_mName);
          const target = PriorityHeroTargets.get(hero.m_mName) ?? defaultCopiesTarget;
          rowsToBuild.push({ safeName: hero.m_mName, isTracked, target });
        }
      }
      frida_java_bridge_default.scheduleOnMainThread(() => {
        try {
          heroSection.clear();
          if (heroMap.size === 0) {
            heroSection.addText("Error: Game data not loaded yet.", 14, "#F44747");
            return;
          }
          if (!battleBridge) {
            heroSection.addText("Waiting for battle... (Bridge not found)", 14, "#F44747");
            return;
          }
          if (rowsToBuild.length === 0) {
            heroSection.addText("No line-up found. Select a Line-up first, then click Refresh.", 14, "#AAAAAA");
            return;
          }
          heroSection.addText("Recommended Heroes", 16, "#00AAFF");
          for (const row of rowsToBuild) {
            const safeName = row.safeName;
            heroSection.addSwitchWithCounter(safeName, row.isTracked, row.target, 1, 13, (checked2, amount) => {
              if (checked2)
                PriorityHeroTargets.set(safeName, amount);
              else
                PriorityHeroTargets.delete(safeName);
            });
          }
        } finally {
          isRefreshing = false;
...
function startCheatHooks() {
  Il2Cpp.perform(() => {
    lib = Il2Cpp.domain.assembly("Assembly-CSharp").image;
    BattleDataClass = lib.class("MCLogicBattleData");
    HeroShopClass = lib.class("UIPanelBattleHeroShop");
    BattleBridgeClass = lib.class("MCBattleBridge");
    hooksReady = true;
    getHeroElements(lib);
    let shopInstance = null;
    const purchasedSlots = /* @__PURE__ */ new Set();
    let lastKnownCoin = -1;
    let lastGCSearch = 0;
    const GC_COOLDOWN_MS = 5e3;
    let gameReady = false;
    function canSearchGC() {
      const now = Date.now();
      if (now - lastGCSearch > GC_COOLDOWN_MS) {
        lastGCSearch = now;
        return true;
      }
      return false;
    ...
    function getBattleData() {
      if (!battleData && canSearchGC()) {
        try {
          const [found] = Il2Cpp.gc.choose(BattleDataClass);
          battleData = found ?? null;
        } catch (e) {
          battleData = null;
        }
      }
    ...
    function getShopInstance() {
      if (!shopInstance && canSearchGC()) {
        try {
          const [found] = Il2Cpp.gc.choose(HeroShopClass);
    ...
    function tryBuySlot(slot) {
      const shop = getShopInstance();
      if (!shop)
        return;
      const keyBoardSelect = shop.tryMethod("KeyBoardShopSelect");
      const buyHero = shop.tryMethod("BuyHero");
      if (keyBoardSelect)
        keyBoardSelect.invoke(slot);
      else if (buyHero)
        buyHero.invoke(slot, false);
    }
    function NeedCopies(ownedCount, safeName) {
      if (PriorityHeroTargets.has(safeName))
        return ownedCount < PriorityHeroTargets.get(safeName);
      return ownedCount < defaultCopiesTarget;
    }
    function ScanAndBuyShop(data, accountId, bridgeInstance = null) {
      let currentCoin = data.method("ILOGIC_GetPlayerCoin").invoke(accountId);
      const bridge = bridgeInstance ?? battleBridge;
      if (!bridge)
        return;
      for (let slot = 0; slot < 5; slot++) {
        if (purchasedSlots.has(slot))
          continue;
        const shopData = data.method("ILOGIC_GetShopItemData").invoke(accountId, slot);
        if (!shopData)
          continue;
        const heroId = shopData.field("m_iHeroId").value;
        if (heroId <= 0)
          continue;
        const isFree = data.method("ILOGIC_IsCurrFreeBuy").invoke(accountId, slot, Il2Cpp.reference(false));
        const price = shopData.field("m_iPrice").value;
        if (isFree || price <= 0) {
          tryBuySlot(slot + 1);
          purchasedSlots.add(slot);
          continue;
        }
        const isRecommended = bridge.method("IsHeroInRecommendLineup").invoke(heroId);
        if (!isRecommended)
          continue;
        const hero = heroMap.get(heroId);
        if (!hero)
          continue;
        const ownedCount = data.method("ILogic_HeroOwnCount").invoke(accountId, heroId);
        if (!NeedCopies(ownedCount, hero.m_mName))
          continue;
        if (currentCoin - price >= KEEP_COIN) {
          tryBuySlot(slot + 1);
          currentCoin -= price;
          purchasedSlots.add(slot);
        }
    ...
    function ShouldStopRefresh(data, accountId) {
      ...
      for (let slot = 0; slot < 5; slot++) {
        ...
        if (isFree)
          return true;
        const price = shopData.field("m_iPrice").value;
        if (price <= 0 && !purchasedSlots.has(slot))
          return true;
        const isRecommended = bridge.method("IsHeroInRecommendLineup").invoke(heroId);
        if (!isRecommended)
          continue;
    ...
    setInterval(() => {
      try {
        ...
        if (!data)
          return;
        const currentCoin = data.method("ILOGIC_GetPlayerCoin").invoke(accountId);
        let hasUnboughtFreeItem = false;
        for (let slot = 0; slot < 5; slot++) {
          if (purchasedSlots.has(slot))
            continue;
          const shopData = data.method("ILOGIC_GetShopItemData").invoke(accountId, slot);
          if (shopData) {
            const price = shopData.field("m_iPrice").value;
            if (price <= 0 && shopData.field("m_iHeroId").value > 0) {
              hasUnboughtFreeItem = true;
              break;
            }
          }
        }
        if (currentCoin > lastKnownCoin || hasUnboughtFreeItem) {
          ScanAndBuyShop(data, accountId);
        }
        lastKnownCoin = currentCoin;
      } catch (e) {
      }
    }, 500);
    setInterval(() => {
      if (!isAuthorized)
        return;
      nextEnemyName = getOpponent();
    }, 2e3);
    BattleBridgeClass.method("OnRefreshShop").implementation = function(isAutoRefresh, dictHeroSlot, sameRefreshHero, isDelayOpenShop, onlyRefreshHero) {
      gameReady = true;
      ...
      if (accountId) {
        const data = getBattleData();
        if (data) {
          lastKnownCoin = data.method("ILOGIC_GetPlayerCoin").invoke(accountId);
          ScanAndBuyShop(data, accountId, this);
        }
      }
      return this.method("OnRefreshShop").invoke(isAutoRefresh, dictHeroSlot, sameRefreshHero, isDelayOpenShop, onlyRefreshHero);
    };
    setInterval(() => {
      try {
        if (!isAuthorized || !gameReady)
          return;
        const accountId = BattleDataClass.field("m_SelfAccID").value;
        if (!accountId) {
          battleData = null;
    ...
    HeroShopClass.method("KeyBoardRefreshShop").implementation = function() {
      const accountId = BattleDataClass.field("m_SelfAccID").value;
      if (accountId) {
        const data = getBattleData();
        if (data && ShouldStopRefresh(data, accountId))
          return;
      }
      this.method("KeyBoardRefreshShop").invoke();
    };
  });
}
function main() {
  frida_java_bridge_default.perform(() => {
    ...
    const displayMetrics = classLoader.Resources.getSystem().getDisplayMetrics();
    const width = displayMetrics.widthPixels.value;
    const height = displayMetrics.heightPixels.value;
    activity.runOnUiThread(frida_java_bridge_default.registerClass({
      name: "com.example.menu.MenuInit",
      implements: [frida_java_bridge_default.use("java.lang.Runnable")],
      methods: {
        run: () => {
          menu = new Menu(classLoader, activity);
          menu.createMenuStart("\u{10300}\u{10314}\u{10315}", 10, "#FFFFFF");
          menu.createMenuLayout("#1C1C1F", 280, 340);
          menu.createMenuBarLayout("#26262B");
          menu.createMenuBarTitle("@AstralCiper", 10, "#FFFFFF");
          menu.createMenuOptionsLayout("#2E7D32", "#26262B");
          menu.addSwitch("Enabled", isAuthorized, (on) => {
            isAuthorized = on;
          });
          menu.addSeekBar("Default Copies Target", defaultCopiesTarget, 1, 10, (value, state) => {
            if (state === "end")
              defaultCopiesTarget = value;
          });
          menu.addOption("refresh", "Refresh Line-Up", {
            on: () => {
              PriorityHeroTargets.clear();
              refreshHeroList();
            },
            off: () => {
              PriorityHeroTargets.clear();
          ...
          heroSection = menu.createDynamicSection();
          heroSection.addText("Tap Refresh Line-Up to load heroes.", 14, "#AAAAAA");
          hud = menu.addOverlayCanvas(width, height);
          hud.startPeriodicUpdate((canvas) => {
            if (!nextEnemyName)
              nextEnemyName = "Waiting...";
            const text = `Next Enemy: ${nextEnemyName}`;
            const approximateCharWidth = 18;
            const estimatedWidth = text.length * approximateCharWidth;
            canvas.drawText(width / 2 - estimatedWidth / 2, height - 30, text, "#FFFF00", 35);
          });
          menu.start();
        }
      }
